...
async def main_async(args):
    ...
    try:
        # Start HTTP server first so we fail fast on port conflicts.
        ...
        # Start background tasks after successful server startup.
        tasks = [
...
def main():
    ...
    # Windows event loops do not support add_signal_handler; fall back to KeyboardInterrupt.
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            loop.add_signal_handler(sig, shutdown, sig)
        except NotImplementedError:
            log.info("Signal handlers are not supported on this platform; use Ctrl+C to stop.")
            break
